...
std::unordered_map<std::string, int> shopping_list(
    {{"apples", 5},
     {"eggs", 12}});
...
int main(int argc, char *argv[])
{
  ...
  std::cout << first << "\n";
  int size = alphabet.size(); // what is capacity
  std::cout << size << "\n";
  for (auto letter : alphabet)
  {
    std::cout << letter;
  ...
  shopping_list["eggs"] = 2; // does update, but will also create if does not exits.
